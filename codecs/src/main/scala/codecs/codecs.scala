...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] = 
    Decoder.fromPartialFunction{
      case Json.Num(a) if a.isValidInt => a.toInt}
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction {
      case lst: Json.Arr => lst.items.flatMap(decoder.decode)
    }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(json) => json.get(name).flatMap(decoder.decode)
      case _ => None
    }
...
trait PersonCodecs {
  ...
  implicit lazy val personDecoder: Decoder[Person] =
    ...
    .transform[Person](pair => Person(pair._1, pair._2))
