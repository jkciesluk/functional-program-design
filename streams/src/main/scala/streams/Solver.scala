...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    (for {
      (newPos, move) <- b.legalNeighbors
    } yield (newPos, move :: history)).to(LazyList)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
      ...
      if(initial.isEmpty) LazyList.empty
        ...
        val next = for {
          ...
          (newBlock, newHistory) <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
        } yield (newBlock, newHistory)
